...
{
    public class AuhtorAppService : IAuthorAppService
    {
        private readonly IAuthorManage _authorManage;
        private readonly IRepository<Author, Guid> _authorRepository;
        private readonly IAbpSession _abpSession;

        public AuhtorAppService(IAuthorManage authorManage, IRepository<Author, Guid> authorRepository, IAbpSession abpSession)
        {
        ...
        {
            var author = Author.Create(input.FirstName, input.LastName, input.BirthDate, input.TenantId);
            var authorResult = _authorManage.Create(author);

            return authorResult;
        }

        public AuthorDto UpdateAuthor(AuthorDto input)
        {
            var author = input.MapTo<Author>();
            var authorResult = _authorManage.Update(author);
